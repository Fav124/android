...
fun DeisaApp(session: SessionManager) {
    ...
    val startDestination = if (session.isLoggedIn()) Routes.MAIN else Routes.LOGIN

    NavHost(navController = rootNav, startDestination = startDestination) {
        composable(Routes.LOGIN) {
            val vm: AuthViewModel = viewModel(factory = AuthViewModel.Factory(session))
            LoginScreen(vm) {
...
private fun MainShell(session: SessionManager, onLogout: () -> Unit) {
    ...
    val masterItems = listOf(
        DrawerNavItem(Routes.SANTRI, "Data Santri", Icons.Default.People, visible = canManageData),
    ...
    val adminItems = listOf(
        DrawerNavItem(Routes.USERS, "Manajemen User", Icons.Default.AdminPanelSettings, visible = isSuperAdmin),
        DrawerNavItem(Routes.APPROVALS, "Persetujuan Akun", Icons.Default.VerifiedUser, visible = isSuperAdmin),
    )
            ...
            ModalDrawerSheet(
                ...
                Column(modifier = Modifier.padding(horizontal = 20.dp, vertical = 24.dp)) {
                    Box(
                        ...
                        Text("DH", color = Primary, fontWeight = FontWeight.Bold, fontSize = 18.sp)
                    }
